(ns lexisnexis.main
  "Kotodama WASM entrypoint for the Lexisnexis clean-room actor (L4) — Clojure port.
  ...
  facts (`lexisnexis.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lexisnexis")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"          :plural "matters"          :id-prefix "lexisnex_mat"
    :fields [:clientId :name :status :openedAt]                          :required [:name :status]
    :coerce {}                                           :refs {}}
   {:entity "Document"        :plural "documents"        :id-prefix "lexisnex_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"lexisnexis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lexisnexis-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lexisnexis-compat/ready)
